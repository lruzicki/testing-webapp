export const mockedProductList = [
  {
    _id: {
      testApp: 'Biodex222',
      testSuite: 'Finance',
      sourceBranch: 'venenatis',
    },
    compatibilities: [
      {
        id: '07112c0a-8263-4717-92ce-c52bca785624',
        buildingBlock: 'mobility_management',
        timestamp: 1644849545000,
        saveTime: 1659380963000,
        testsPassed: 2,
        testsFailed: 2,
        compatibility: 0.2455,
      },
      {
        id: '07112c0a-8263-4717-92ce-c52bca785624',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1644849545000,
        saveTime: 1659380963000,
        testsPassed: 2,
        testsFailed: 2,
        compatibility: 0.9455,
      },
      {
        id: 'ba37d9df-c353-4f79-b86f-6416d5b9ca09',
        buildingBlock: 'client_case_management',
        timestamp: 1645985463000,
        saveTime: 1675370507000,
        testsPassed: 6,
        testsFailed: 4,
        compatibility: 0.2455,
      },
      {
        id: 'ba37d9df-c353-4f79-b86f-6416d5b9ca09',
        buildingBlock: 'client_case_management',
        timestamp: 1645985463000,
        saveTime: 1675370507000,
        testsPassed: 6,
        testsFailed: 4,
        compatibility: 0.7755,
      },
      {
        id: '503d1764-ec33-4763-bd15-4168996f41b2',
        buildingBlock: 'artificial_intelligence',
        timestamp: 1672130537000,
        saveTime: 1657518851000,
        testsPassed: 14,
        testsFailed: 8,
        compatibility: 0.2455,
      },
      {
        id: '07112c0a-8263-4717-92ce-c52bca785624',
        buildingBlock: 'mobility_management',
        timestamp: 1644849545000,
        saveTime: 1659380963000,
        testsPassed: 2,
        testsFailed: 2,
        compatibility: 0.7455,
      },
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.5455,
      },
      {
        id: '0ba8dcdc-0c12-4a7e-b9c6-e7674379cf15',
        buildingBlock: 'consent_management',
        timestamp: 1647762312000,
        saveTime: 1673582060000,
        testsPassed: 8,
        testsFailed: 5,
        compatibility: 0.3455,
      },
    ],
    overallCompatibility: 1,
  },
  {
    _id: { testApp: 'Subin', testSuite: 'Technology', sourceBranch: 'a' },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.3455,
      },
      {
        id: '0ba8dcdc-0c12-4a7e-b9c6-e7674379cf15',
        buildingBlock: 'consent_management',
        timestamp: 1647762312000,
        saveTime: 1673582060000,
        testsPassed: 8,
        testsFailed: 5,
        compatibility: 0.3455,
      },
    ],
    overallCompatibility: 0.1323,
  },
  {
    _id: {
      testApp: 'Tampflex',
      testSuite: 'Consumer Services',
      sourceBranch: 'libero',
    },
    compatibilities: [
      {
        id: '07112c0a-8263-4717-92ce-c52bca785624',
        buildingBlock: 'mobility_management',
        timestamp: 1644849545000,
        saveTime: 1659380963000,
        testsPassed: 2,
        testsFailed: 2,
        compatibility: 0.2455,
      },
      {
        id: 'ba37d9df-c353-4f79-b86f-6416d5b9ca09',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1645985463000,
        saveTime: 1675370507000,
        testsPassed: 6,
        testsFailed: 4,
        compatibility: 0.2455,
      },
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
      {
        id: '0ba8dcdc-0c12-4a7e-b9c6-e7674379cf15',
        buildingBlock: 'consent_management',
        timestamp: 1647762312000,
        saveTime: 1673582060000,
        testsPassed: 8,
        testsFailed: 5,
        compatibility: 0.2455,
      },
      // {
      //   id: '0ba8dcdc-0c12-4a7e-b9c6-e7674379cf15',
      //   buildingBlock: 'consent_management',
      //   timestamp: 1647762312000,
      //   saveTime: 1673582060000,
      //   testsPassed: 8,
      //   testsFailed: 5,
      // },
    ],
    overallCompatibility: 0.5323,
  },
  {
    _id: {
      testApp: 'Zamit',
      testSuite: 'Consumer Services',
      sourceBranch: 'duis',
    },
    compatibilities: [
      {
        id: '0ba8dcdc-0c12-4a7e-b9c6-e7674379cf15',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1647762312000,
        saveTime: 1673582060000,
        testsPassed: 8,
        testsFailed: 5,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.7723,
  },
  {
    _id: {
      testApp: 'Zoolab',
      testSuite: 'Public Utilities',
      sourceBranch: 'sapien',
    },
    compatibilities: [
      {
        id: '53f5dd11-767c-4944-9055-72d5b1f4447c',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1659961473000,
        saveTime: 1670701134000,
        testsPassed: 10,
        testsFailed: 6,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Job',
      testSuite: 'Consumer Non-Durables',
      sourceBranch: 'donec',
    },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Cardify',
      testSuite: 'Consumer Services',
      sourceBranch: 'venenatis',
    },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Treeflex', testSuite: 'n/a', sourceBranch: 'porttitor' },
    compatibilities: [
      {
        id: '503d1764-ec33-4763-bd15-4168996f41b2',
        buildingBlock: 'artificial_intelligence',
        timestamp: 1672130537000,
        saveTime: 1657518851000,
        testsPassed: 14,
        testsFailed: 8,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Biodex', testSuite: 'Finance', sourceBranch: 'venenatis' },
    compatibilities: [
      {
        id: '07112c0a-8263-4717-92ce-c52bca785624',
        buildingBlock: 'mobility_management',
        timestamp: 1644849545000,
        saveTime: 1659380963000,
        testsPassed: 2,
        testsFailed: 2,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Subin', testSuite: 'Technology', sourceBranch: 'a' },
    compatibilities: [
      {
        id: '43e0fe2f-dbcd-44f5-931d-b0981cf98b10',
        buildingBlock: 'artificial_intelligence',
        timestamp: 1672527384000,
        saveTime: 1645552798000,
        testsPassed: 4,
        testsFailed: 3,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Tampflex',
      testSuite: 'Consumer Services',
      sourceBranch: 'libero',
    },
    compatibilities: [
      {
        id: 'ba37d9df-c353-4f79-b86f-6416d5b9ca09',
        buildingBlock: 'client_case_management',
        timestamp: 1645985463000,
        saveTime: 1675370507000,
        testsPassed: 6,
        testsFailed: 4,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Zamit',
      testSuite: 'Consumer Services',
      sourceBranch: 'duis',
    },
    compatibilities: [
      {
        id: '0ba8dcdc-0c12-4a7e-b9c6-e7674379cf15',
        buildingBlock: 'consent_management',
        timestamp: 1647762312000,
        saveTime: 1673582060000,
        testsPassed: 8,
        testsFailed: 5,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Zoolab',
      testSuite: 'Public Utilities',
      sourceBranch: 'sapien',
    },
    compatibilities: [
      {
        id: '53f5dd11-767c-4944-9055-72d5b1f4447c',
        buildingBlock: 'data_collection',
        timestamp: 1659961473000,
        saveTime: 1670701134000,
        testsPassed: 10,
        testsFailed: 6,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Job',
      testSuite: 'Consumer Non-Durables',
      sourceBranch: 'donec',
    },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Cardify',
      testSuite: 'Consumer Services',
      sourceBranch: 'venenatis',
    },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Treeflex', testSuite: 'n/a', sourceBranch: 'porttitor' },
    compatibilities: [
      {
        id: '503d1764-ec33-4763-bd15-4168996f41b2',
        buildingBlock: 'artificial_intelligence',
        timestamp: 1672130537000,
        saveTime: 1657518851000,
        testsPassed: 14,
        testsFailed: 8,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Biodex', testSuite: 'Finance', sourceBranch: 'venenatis' },
    compatibilities: [
      {
        id: '07112c0a-8263-4717-92ce-c52bca785624',
        buildingBlock: 'mobility_management',
        timestamp: 1644849545000,
        saveTime: 1659380963000,
        testsPassed: 2,
        testsFailed: 2,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Subin', testSuite: 'Technology', sourceBranch: 'a' },
    compatibilities: [
      {
        id: '43e0fe2f-dbcd-44f5-931d-b0981cf98b10',
        buildingBlock: 'artificial_intelligence',
        timestamp: 1672527384000,
        saveTime: 1645552798000,
        testsPassed: 4,
        testsFailed: 3,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Tampflex',
      testSuite: 'Consumer Services',
      sourceBranch: 'libero',
    },
    compatibilities: [
      {
        id: 'ba37d9df-c353-4f79-b86f-6416d5b9ca09',
        buildingBlock: 'client_case_management',
        timestamp: 1645985463000,
        saveTime: 1675370507000,
        testsPassed: 6,
        testsFailed: 4,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Zamit',
      testSuite: 'Consumer Services',
      sourceBranch: 'duis',
    },
    compatibilities: [
      {
        id: '0ba8dcdc-0c12-4a7e-b9c6-e7674379cf15',
        buildingBlock: 'consent_management',
        timestamp: 1647762312000,
        saveTime: 1673582060000,
        testsPassed: 8,
        testsFailed: 5,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
];

export const moreMockedProductList = [
  {
    _id: {
      testApp: 'Zoolab',
      testSuite: 'Public Utilities',
      sourceBranch: 'sapien',
    },
    compatibilities: [
      {
        id: '53f5dd11-767c-4944-9055-72d5b1f4447c',
        buildingBlock: 'data_collection',
        timestamp: 1659961473000,
        saveTime: 1670701134000,
        testsPassed: 10,
        testsFailed: 6,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Job',
      testSuite: 'Consumer Non-Durables',
      sourceBranch: 'donec',
    },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Cardify',
      testSuite: 'Consumer Services',
      sourceBranch: 'venenatis',
    },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Treeflex', testSuite: 'n/a', sourceBranch: 'porttitor' },
    compatibilities: [
      {
        id: '503d1764-ec33-4763-bd15-4168996f41b2',
        buildingBlock: 'artificial_intelligence',
        timestamp: 1672130537000,
        saveTime: 1657518851000,
        testsPassed: 14,
        testsFailed: 8,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Biodex', testSuite: 'Finance', sourceBranch: 'venenatis' },
    compatibilities: [
      {
        id: '07112c0a-8263-4717-92ce-c52bca785624',
        buildingBlock: 'mobility_management',
        timestamp: 1644849545000,
        saveTime: 1659380963000,
        testsPassed: 2,
        testsFailed: 2,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Subin', testSuite: 'Technology', sourceBranch: 'a' },
    compatibilities: [
      {
        id: '43e0fe2f-dbcd-44f5-931d-b0981cf98b10',
        buildingBlock: 'artificial_intelligence',
        timestamp: 1672527384000,
        saveTime: 1645552798000,
        testsPassed: 4,
        testsFailed: 3,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Tampflex',
      testSuite: 'Consumer Services',
      sourceBranch: 'libero',
    },
    compatibilities: [
      {
        id: 'ba37d9df-c353-4f79-b86f-6416d5b9ca09',
        buildingBlock: 'client_case_management',
        timestamp: 1645985463000,
        saveTime: 1675370507000,
        testsPassed: 6,
        testsFailed: 4,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Zamit',
      testSuite: 'Consumer Services',
      sourceBranch: 'duis',
    },
    compatibilities: [
      {
        id: '0ba8dcdc-0c12-4a7e-b9c6-e7674379cf15',
        buildingBlock: 'consent_management',
        timestamp: 1647762312000,
        saveTime: 1673582060000,
        testsPassed: 8,
        testsFailed: 5,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Zoolab',
      testSuite: 'Public Utilities',
      sourceBranch: 'sapien',
    },
    compatibilities: [
      {
        id: '53f5dd11-767c-4944-9055-72d5b1f4447c',
        buildingBlock: 'data_collection',
        timestamp: 1659961473000,
        saveTime: 1670701134000,
        testsPassed: 10,
        testsFailed: 6,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Job',
      testSuite: 'Consumer Non-Durables',
      sourceBranch: 'donec',
    },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Cardify',
      testSuite: 'Consumer Services',
      sourceBranch: 'venenatis',
    },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Treeflex', testSuite: 'n/a', sourceBranch: 'porttitor' },
    compatibilities: [
      {
        id: '503d1764-ec33-4763-bd15-4168996f41b2',
        buildingBlock: 'artificial_intelligence',
        timestamp: 1672130537000,
        saveTime: 1657518851000,
        testsPassed: 14,
        testsFailed: 8,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Biodex', testSuite: 'Finance', sourceBranch: 'venenatis' },
    compatibilities: [
      {
        id: '07112c0a-8263-4717-92ce-c52bca785624',
        buildingBlock: 'mobility_management',
        timestamp: 1644849545000,
        saveTime: 1659380963000,
        testsPassed: 2,
        testsFailed: 2,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Subin', testSuite: 'Technology', sourceBranch: 'a' },
    compatibilities: [
      {
        id: '43e0fe2f-dbcd-44f5-931d-b0981cf98b10',
        buildingBlock: 'artificial_intelligence',
        timestamp: 1672527384000,
        saveTime: 1645552798000,
        testsPassed: 4,
        testsFailed: 3,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Tampflex',
      testSuite: 'Consumer Services',
      sourceBranch: 'libero',
    },
    compatibilities: [
      {
        id: 'ba37d9df-c353-4f79-b86f-6416d5b9ca09',
        buildingBlock: 'client_case_management',
        timestamp: 1645985463000,
        saveTime: 1675370507000,
        testsPassed: 6,
        testsFailed: 4,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Zamit',
      testSuite: 'Consumer Services',
      sourceBranch: 'duis',
    },
    compatibilities: [
      {
        id: '0ba8dcdc-0c12-4a7e-b9c6-e7674379cf15',
        buildingBlock: 'consent_management',
        timestamp: 1647762312000,
        saveTime: 1673582060000,
        testsPassed: 8,
        testsFailed: 5,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Zoolab',
      testSuite: 'Public Utilities',
      sourceBranch: 'sapien',
    },
    compatibilities: [
      {
        id: '53f5dd11-767c-4944-9055-72d5b1f4447c',
        buildingBlock: 'data_collection',
        timestamp: 1659961473000,
        saveTime: 1670701134000,
        testsPassed: 10,
        testsFailed: 6,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Job',
      testSuite: 'Consumer Non-Durables',
      sourceBranch: 'donec',
    },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: {
      testApp: 'Cardify',
      testSuite: 'Consumer Services',
      sourceBranch: 'venenatis',
    },
    compatibilities: [
      {
        id: '6b696f8f-8d3f-4322-a269-8d49970849f0',
        buildingBlock: 'analytics_and_business_intel',
        timestamp: 1663755505000,
        saveTime: 1670490781000,
        testsPassed: 12,
        testsFailed: 7,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
  {
    _id: { testApp: 'Treeflex', testSuite: 'n/a', sourceBranch: 'porttitor' },
    compatibilities: [
      {
        id: '503d1764-ec33-4763-bd15-4168996f41b2',
        buildingBlock: 'artificial_intelligence',
        timestamp: 1672130537000,
        saveTime: 1657518851000,
        testsPassed: 14,
        testsFailed: 8,
        compatibility: 0.2455,
      },
    ],
    overallCompatibility: 0.2323,
  },
];
